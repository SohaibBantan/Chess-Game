...
public class Pawn extends Piece {
    ...
    public void promotePawn(Board board, int row, int col, boolean isBlack) {
        ...
        if ((isBlack && row == 7) || (!isBlack && row == 0)) {
            System.out.println("Bishop,Knight,Rook, or Queen");
            String wantPromote = inp.next();
            if (wantPromote.equals("Bishop") || wantPromote.equals("bishop")) { // Bishop promotion
                Bishop bishop = new Bishop(row, col, isBlack);
                board.setPiece(row, col, bishop);
            }
            if (wantPromote.equals("Knight") || wantPromote.equals("knight")) { //Knight promotion
                Knight knight = new Knight(row, col, isBlack);
                board.setPiece(row, col, knight);
            }
            if (wantPromote.equals("Rook") || wantPromote.equals("rook")) { //Rock's promotion
                Rook rook = new Rook(row, col, isBlack);
                board.setPiece(row, col, rook);

            }
            if (wantPromote.equals("Queen") || wantPromote.equals("queen")) { //Queen promotion
